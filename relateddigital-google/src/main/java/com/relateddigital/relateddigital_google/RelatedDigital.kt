...
object RelatedDigital {
    // init() runs on the main thread while FCM delivers onNewToken on a background thread, so both
    // references have to be published safely between them.
    @Volatile
    ...
    fun setIsPushNotificationEnabled(
        ...
        if (googleAppAlias.isEmpty()) {
            Log.e(
                PushDiagnostics.LOG_TAG,
                "setIsPushNotificationEnabled was called with an empty googleAppAlias. Pass the " +
                        "appAlias (appKey) given to you by Related Digital, otherwise the " +
        ...
        if (token.isEmpty()) {
            Log.w(
                ...
                "setIsPushNotificationEnabled was called with an empty token. Obtain it with " +
                        "FirebaseMessaging.getInstance().token and only call this method from the " +
                        "success branch of that listener. A previously stored token is kept."
            )
        }
        ...
        currentModel.setHuaweiAppAlias(context, "")
        ...
        if (token.isEmpty()) {
            Log.w(
                ...
                "setToken was called with an empty token; a previously stored token is kept. " +
                        "Obtain the token with FirebaseMessaging.getInstance().token first."
            )
        }
     ...
     */
    /**
     * DataStore'dan push mesajlarını asenkron olarak alır.
     * Eski Thread yapısı yerine Activity'nin yaşam döngüsüne duyarli, güvenli Coroutine kullanır.
     */
    @JvmStatic
