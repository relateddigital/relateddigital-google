...
import com.relateddigital.relateddigital_google.inapp.inappmessages.*
...
class InAppManager(
        ...
        parent.runOnUiThread(Runnable {
            ...
            try {
                ...
                when (inAppMessage.mActionData!!.mMsgType) {
                    ...
                    InAppNotificationType.HALF_SCREEN.toString() -> {
                        stateId = getStateId(parent, inAppMessage)
                        inAppUpdateDisplayState = InAppUpdateDisplayState.claimDisplayState(
                                stateId
                        )
                        if (inAppUpdateDisplayState == null) {
                            ...
                            openInAppHalfScreenFragment(stateId, parent, inAppUpdateDisplayState)
                        }
                    }
                    ...
                    InAppNotificationType.FULLSCREEN_POPUP.toString() -> {
                        openFullscreenPopupActivity(parent, getStateId(parent, inAppMessage))
                    }
    ...
    private fun openInAppHalfScreenFragment(
            stateID: Int,
            parent: Activity,
            inAppUpdateDisplayState: InAppUpdateDisplayState
    ) {
    ...
    private fun openFullscreenPopupActivity(parent: Activity, inAppData: Int) {
        val intent = Intent(parent.applicationContext, FullscreenPopupActivity::class.java)
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        intent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT)
        intent.putExtra(Constants.INTENT_ID_KEY, inAppData)
        parent.startActivity(intent)
