package com.relateddigital.relateddigital_google.model

import com.google.gson.annotations.SerializedName
import java.io.Serializable

class ShakeToWinMailSubscriptionForm : Serializable {
    @SerializedName("title")
    var title: String? = null

    @SerializedName("message")
    var message: String? = null

    @SerializedName("placeholder")
    var placeholder: String? = null

    @SerializedName("button_label")
    var buttonLabel: String? = null

    @SerializedName("consent_text")
    var consentText: String? = null

    @SerializedName("invalid_email_message")
    var invalidEmailMessage: String? = null

    @SerializedName("success_message")
    var successMessage: String? = null

    @SerializedName("emailpermit_text")
    var emailPermitText: String? = null

    @SerializedName("check_consent_message")
    var checkConsentMessage: String? = null

}